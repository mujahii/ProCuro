...
export default function AddressSwitcher({ onAddNew }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 bg-lionsmane hover:bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-sm transition-colors max-w-xs"
      >
        ...
        <div className="absolute top-full left-0 mt-1 w-64 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1">
          {addresses.map(addr => (
            <button
              ...
              className={`w-full text-left px-4 py-3 hover:bg-lionsmane flex items-start gap-3 ${selectedAddress?.id === addr.id ? 'bg-primary-50' : ''}`}
            >
          ...
          <div className="border-t border-gray-100 mt-1 pt-1">
            <button
              ...
              className="w-full text-left px-4 py-2.5 hover:bg-lionsmane flex items-center gap-2 text-sm text-primary font-medium"
            >
