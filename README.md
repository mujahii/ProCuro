...
**Last Updated:** 2026-05-29 14:58 (MYT — Kuala Lumpur)
...
|---|---|---|
| LandingPage2 | `/homepage-2` | **Webyan.sa page structure with ProCuro color palette.** Self-contained component using `wy-` prefixed CSS injected via `<style>` block (zero Tailwind collision). Fonts: IBM Plex Sans (body) + Plus Jakarta Sans (display). Colors: ProCuro tokens — `midnight #083A4F`, `marigold #A58D66`, `celeste #C0D5D6`, `herb #407E8C`, `lionsmane #E5E1DD`. Hero: identical structure to original LandingPage — dark `#052532` background, 6 rising blob orbs (`wy-orb` / `wy-rise` keyframes: `translateY(680px → -680px)`, mobile `riseBlobMobile` variant), `rgba(5,37,50,0.60)` overlay, 4-zone centered `justify-between` flex-col layout (badge top → H1 → subtitle+CTAs → trust badges). Frosted-glass sticky header: `backdrop-filter: blur(16px)`, `.wy-scrolled` on scroll. Mobile: right-slide drawer (300 px, `right:-340px → 0`). Stats bar: `IntersectionObserver` count-up, gradient text. Product horizontal scroll (260 px cards, midnight `+` button). Supplier horizontal scroll (180 px cards, sand/celeste Halal badge). How It Works: 3 lift cards, celeste icon circle, number badge. CTA/app section: `midnight-dark` bg, marigold CTA button, white-outline ghost button, 3 store badges. Dark footer: 4-col grid, sand/celeste link hover. Scroll-reveal: `.wy-reveal → .wy-reveal--visible` (threshold 0.12, stagger `d1–d5`). All ProCuro data logic preserved. Route lazy-loaded. |
