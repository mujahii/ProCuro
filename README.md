...
| DashboardPage | `/admin/dashboard` | Platform KPIs (title: "Overview"): GMV, user count, order count. Charts: Platform GMV Over Time (filtered), **Orders by Status (column/bar chart — one bar per status, colour-coded)**, **Orders by Payment Type** (COD vs Bank Transfer donut — left column), **User Growth** (cumulative line chart — right column), **City Comparison Radar** + **Germany Dot Map**. Both location charts merge two sources — each user's home city from `supplier_profiles`/`owner_profiles` (where most users live, with coordinates) **and** every row in the `addresses` table (relocations) — deduplicated per `(user, city)`. So a relocated user appears as one dot per distinct city, and every listed city shows on the radar (not just cities that have address rows). AI summary. Week/month/year + custom date-range filter on all charts. |
...
- `CityComparisonRadar` — Polar/radar chart with one axis per city, two overlaid series (Suppliers + Owners). Shows up to 12 cities (every city in the dataset; capped only for legibility). Intentionally a different chart family from everything else on the dashboard. **Domain is dynamic**: computed as `Math.ceil(max(all series values) × 1.15)` so the radar shape proportionally reflects actual differences rather than being artificially scaled.
