...
export default function RegisterOwnerPage() {
  ...
  useEffect(() => {
    async function checkOAuthReturn() {
      if (localStorage.getItem('procuro_oauth_role') !== 'restaurant_owner') return
      ...
      try {
        ...
        const { data, error } = await supabase.rpc('create_profile_from_oauth', {
          p_role: 'restaurant_owner',
          p_full_name: fullName,
        })
        if (error) throw error
        await refreshProfile()
        navigate('/owner/store', { replace: true })
      } catch (err) {
  ...
  async function handleOAuth(provider) {
    localStorage.setItem('procuro_oauth_role', 'restaurant_owner')
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        <div className="p-8">
          ...
          <div className="flex items-center mb-6">
            ...
            <span className="px-3 text-xs text-slate-400 font-medium">OR SIGN UP WITH EMAIL</span>
            <div className="flex-1 border-t border-slate-200" />
          </div>
