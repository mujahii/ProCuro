    ...
    <div className="min-h-screen bg-lionsmane flex flex-col pt-16">
      ...
      <main className="max-w-3xl mx-auto px-4 py-12 flex-grow w-full">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-black text-slate-900">{t('careersTitle')}</h1>
        </div>
        <p className="text-slate-500 mb-8">{t('careersSubtitle')}</p>

        <div className="bg-lionsmane border border-celeste rounded-2xl p-6 mb-8">
          <h2 className="font-bold text-midnight mb-1">{t('careersWhyTitle')}</h2>
          <p className="text-sm text-midnight leading-relaxed">{t('careersWhyDesc')}</p>
        </div>

        <h2 className="text-lg font-bold text-slate-900 mb-4">{t('careersOpenPositions')}</h2>
        <div className="space-y-4 mb-10">
          {openings.map(job => (
            <div key={job.titleKey} className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
              <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                <h3 className="font-bold text-slate-900">{t(job.titleKey)}</h3>
                <span className="text-xs font-semibold bg-celeste text-midnight-dark px-2.5 py-1 rounded-full">{t(job.typeKey)}</span>
              </div>
              <p className="flex items-center gap-1.5 text-xs text-slate-400 mb-2">
                <MapPin className="w-3.5 h-3.5" /> {t(job.locationKey)}
              </p>
              <p className="text-sm text-slate-600 leading-relaxed">{t(job.descKey)}</p>
              <a
                href="mailto:support@procuro.com?subject=Job Application"
                className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-midnight hover:text-midnight-dark transition-colors"
              >
        ...
        <div className="bg-slate-100 rounded-xl p-5 text-center">
          <p className="text-sm text-slate-600 mb-1">{t('careersNoRole')}</p>
