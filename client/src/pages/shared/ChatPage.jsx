...
import { Send, MessageSquare, ArrowLeft, Package, Shield, Paperclip, FileText, MoreVertical, Pin, Trash2, Loader2, X, MapPin, User, Phone, Store, Flag, CheckCircle, ArrowUpRight } from 'lucide-react'
...
function resolveSupplierAvatar(path, supabaseClient) {
  if (!path) return null
  if (path.startsWith('http')) return path
  return supabaseClient.storage.from('avatars').getPublicUrl(path).data?.publicUrl || null
}
...
export default function ChatPage() {
  ...
  const [ownerShowReport, setOwnerShowReport] = useState(false)
  const [supplierProfileModal, setSupplierProfileModal] = useState(null)
  const [supplierProfileData, setSupplierProfileData] = useState(null)
  const [supplierProfileLoading, setSupplierProfileLoading] = useState(false)
  const [supplierShowReport, setSupplierShowReport] = useState(false)
  ...
  async function fetchAndShowOwnerProfile(owner) {
    ...
    setOwnerShowReport(false)
    const [{ data: userData }, { data: profileData }] = await Promise.all([
      supabase.from('users').select('full_name, email, avatar_url, phone').eq('id', owner.id).single(),
      supabase.from('owner_profiles').select('restaurant_name, bio, city').eq('user_id', owner.id).maybeSingle(),
    ])
    setOwnerProfileFull({
      id: owner.id,
  ...
  async function fetchAndShowSupplierProfile(supplier) {
    setSupplierProfileModal(supplier)
    setSupplierProfileData(null)
    setSupplierProfileLoading(true)
    setSupplierShowReport(false)
    const { data } = await supabase
      .from('supplier_profiles')
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 8.5rem)' }}>
        ...
        {!showingAdmin && selectedConv ? (
          <div className="flex-1 flex flex-col min-w-0">
            <div className="p-4 border-b border-slate-100 flex items-center gap-3 flex-shrink-0">
              ...
              <button
                onClick={() => {
                  if (role === 'restaurant_owner' && selectedConv.supplier?.id) fetchAndShowSupplierProfile(selectedConv.supplier)
      ...
      {/* Owner profile modal (for supplier view) — matches OwnerProfileModal from orders */}
      {ownerProfileModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4 py-6">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-midnight px-5 py-5 flex items-center gap-4 relative flex-shrink-0">
              <button onClick={() => { setOwnerProfileModal(null); setOwnerProfileFull(null); setOwnerShowReport(false) }} className="absolute top-3 right-3 text-white/70 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="min-w-0 flex-1 pr-6">
                ...
                {(ownerProfileFull?.restaurant_name || ownerProfileModal.restaurant_name) && (ownerProfileFull?.full_name || ownerProfileModal.full_name) && (
                  <p className="text-celeste text-xs truncate">{ownerProfileFull?.full_name || ownerProfileModal.full_name}</p>
                )}
              </div>
            </div>
                ...
                <>
                  {ownerProfileFull?.bio && (
                    <div className="p-3 bg-lionsmane rounded-xl">
                      ...
                      <p className="text-sm text-slate-700">{ownerProfileFull.bio}</p>
                    </div>
                  )}
                  {ownerProfileFull?.phone && (
                    <div className="flex items-center gap-3 p-3 bg-lionsmane rounded-xl">
                      ...
                      <div>
                        ...
                        <a href={`tel:${ownerProfileFull.phone}`} className="text-sm font-medium text-herb hover:text-herb-dark hover:underline">
                          {fmtPhone(ownerProfileFull.phone)}
                        </a>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="flex items-start gap-3 p-3 bg-lionsmane rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(ownerProfileFull?.city || ownerProfileModal.city).split(',').map(c => c.trim()).filter(Boolean).map((c, i) => (
                  ...
                  {!ownerProfileFull?.bio && !ownerProfileFull?.phone && !(ownerProfileFull?.city || ownerProfileModal.city) && (
            ...
            <div className="px-4 pb-4 pt-2 space-y-2 flex-shrink-0 border-t border-slate-100 bg-white">
              <button
                onClick={() => setOwnerShowReport(true)}
          ...
          {ownerShowReport && ownerProfileModal && (
            <ReportModal
              type="user"
              targetId={ownerProfileModal.id}
              targetName={(ownerProfileFull?.restaurant_name || ownerProfileModal.restaurant_name) || (ownerProfileFull?.full_name || ownerProfileModal.full_name) || 'Restaurant Owner'}
              onClose={() => setOwnerShowReport(false)}
            />
          )}
        </div>
      )}

      {/* Supplier profile modal (for restaurant owner view) — matches SupplierProfileModal from orders */}
      {supplierProfileModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4 py-6">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div className="bg-midnight px-6 py-10 text-center relative">
              <button onClick={() => { setSupplierProfileModal(null); setSupplierProfileData(null); setSupplierShowReport(false) }} className="absolute top-4 right-4 text-white/70 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
              {supplierProfileData?.avatar_url ? (
                <img
                  src={resolveSupplierAvatar(supplierProfileData.avatar_url, supabase)}
                  alt={supplierProfileData.business_name}
              ...
              <h2 className="text-2xl font-bold text-white">{supplierProfileData?.business_name || supplierProfileModal.business_name || 'Supplier'}</h2>
              <div className="flex items-center justify-center gap-2 mt-2 flex-wrap">
                {supplierProfileData?.category && (
                  <span className="text-xs text-slate-300 bg-white/10 px-2.5 py-1 rounded-full">{supplierProfileData.category}</span>
                )}
                {supplierProfileData?.is_verified && (
            ...
            <div className="p-5 space-y-3">
              {!supplierProfileData && <div className="flex justify-center py-4"><Loader2 className="w-5 h-5 animate-spin text-slate-400" /></div>}
              {supplierProfileData?.description && (
                <p className="text-sm text-slate-600 leading-relaxed">{supplierProfileData.description}</p>
              )}
              {supplierProfileData?.city && (
                <div className="flex items-center gap-3 p-3 bg-lionsmane rounded-xl">
                  <MapPin className="w-4 h-4 text-slate-400 flex-shrink-0" />
                  <p className="text-sm font-medium text-slate-700">{supplierProfileData.city}</p>
                </div>
              )}
            </div>
            <div className="px-5 pb-5 space-y-3">
              <button
                onClick={() => { setSupplierProfileModal(null); navigate(`/supplier/${supplierProfileModal.id}`) }}
              ...
              <button
                onClick={() => setSupplierShowReport(true)}
                className="w-full py-2 flex items-center justify-center gap-2 text-xs text-slate-400 hover:text-red-500 transition-colors font-medium"
              >
          ...
          {supplierShowReport && supplierProfileModal && (
            <ReportModal
              type="supplier"
              targetId={supplierProfileModal.id}
              targetName={supplierProfileData?.business_name || supplierProfileModal.business_name || 'Supplier'}
              onClose={() => setSupplierShowReport(false)}
            />
          )}
        </div>
      )}
