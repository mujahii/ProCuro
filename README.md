...
**Last Updated:** 2026-05-28 14:23 (MYT — Kuala Lumpur)
...
|---|---|---|
| LandingPage2 | `/homepage-2` | **Webyan.sa design-system variant** of the main homepage. Shares identical ProCuro data logic (Supabase `products` + `supplier_profiles` queries, `useAuth` redirect, `useLanguage` i18n) but is styled with a fully self-contained `wy-` namespaced CSS system injected via a `<style>` block — zero collision with Tailwind utilities or ProCuro color tokens. Design system: IBM Plex Sans (body) + Plus Jakarta Sans (display headings); brand palette `--wy-navy: #263c84` + `--wy-cyan: #24c2ec`; frosted-glass sticky header (`backdrop-filter: blur(16px)`) with `.wy-scrolled` class applied on scroll; 2-column hero section (text left, 3D perspective mockup card right — `rotateY(-6deg) rotateX(3deg)` animated float); animated stats bar with `IntersectionObserver`-triggered count-up; horizontal product scroll with cyan gradient add-button; auto-fill supplier grid with Halal badge; dark navy gradient CTA banner with radial glow overlays and app-store buttons; 4-column dark footer. Scroll-reveal: `.wy-reveal` → `.wy-reveal--visible` via `IntersectionObserver` (threshold 0.12) with stagger delay classes `wy-reveal-d1–d5`. Mobile (≤ 900 px): desktop nav hidden, right-slide drawer (`right: -340px → right: 0`) with overlay toggled by hamburger button; hero visual card hidden below 900 px. Route is lazy-loaded via `React.lazy()`. |
| LandingPage | `/` | Marketing homepage with hero, features, and CTA. Hero section background uses **six rising blob orbs** animated with `riseBlob` (`translateY` from +680 px to −680 px over 15 s, linear infinite). The six orbs: marigold `#A58D66` at 0 s (far left), celeste `#C0D5D6` at −3.75 s (right-center), herb teal `#5E96A4` at −7.5 s (center), lionsmane cream `#BFA988` at −11.25 s (far right), rose mauve `#B07B8B` at −2 s (left-center), and light purple/lavender `#B19CD9` at −5 s (far right edge). On mobile (`< sm`), orb sizes are capped at `w-20–w-32` with `blur-[22px–32px]` to prevent them from filling the entire viewport; desktop retains `sm:w-[18rem–26rem]` with `sm:blur-[60px–80px]`. Blur uses Tailwind `blur-[Npx]` class (not inline style) to enable responsive `sm:` overrides. Each orb is heavily blurred and a `bg-[#052532]/60` dark overlay keeps text legible. Page top offset uses `calc(4rem + var(--sat, 0px))` so iOS safe-area inset is included and the hero badge clears the navbar on mobile; desktop behaviour unchanged (`--sat` = 0 px). Below the How It Works section, a **Mobile App Download** strip shows three store badges (App Store, Google Play, AppGallery) as white `flex-1` buttons with a border and shadow. Clicking any badge opens a "Coming Soon" modal overlay (tap outside or OK to dismiss). The Google Play icon uses the official 4-colour triangle paths; the Huawei AppGallery icon uses 4 coloured ellipses forming a petal/flower pattern. All visible strings (section heading, tagline, badge sub-labels, modal title and body) are fully i18n'd via `t()` keys (`mobileComingToMobile`, `mobileTakeEverywhere`, `mobileTagline`, `mobileDownloadOn`, `mobileGetItOn`, `mobileExploreOn`, `comingSoonTitle`, `comingSoonDesc`) with both EN and DE translations in `LanguageContext.jsx`. Store names (App Store, Google Play, AppGallery) remain in English as proper nouns. |
...
- `OrdersByStatusChart` — **Column/bar chart** showing order-split status distribution (admin) — one bar per status, each colour-coded using brand palette: marigold `#A58D66` for pending, herb `#407E8C` for confirmed, herb-light `#5E96A4` for in-transit/shipped, midnight `#083A4F` for delivered/completed, red `#EF4444` for cancelled; x-axis labels angled −35°; replaced the previous Treemap.
